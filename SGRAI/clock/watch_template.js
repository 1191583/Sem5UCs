...
export default class Watch extends THREE.Group {
  constructor(
    ...
    var inc = twoPi / 12;
    for (let i = 0; i < 12; i++) {
      var x = radius2 * Math.cos(i * inc);
      var y = radius2 * Math.sin(i * inc);
      points.push(new THREE.Vector2(x, y));

      x = radius0 * Math.cos(i * inc);
      y = radius0 * Math.sin(i * inc);
      points.push(new THREE.Vector2(x, y));
    }
    ...
    for (let i = 0; i < 60; i++) {
      ...
      x = radius1 * Math.cos(i * inc);
      y = radius1 * Math.sin(i * inc);
      points.push(new THREE.Vector2(x, y));
    }
    ...
    points.push(new THREE.Vector2(0.5 * radius, 0));
    ...
    points = [];
    points.push(new THREE.Vector2(0, 0));
    points.push(new THREE.Vector2(0.7 * radius, 0));
  ...
  update() {
    ...
    angle =
      Math.PI / 2.0 -
      (2.0 * Math.PI * time[1]) / 60.0 -
      (2.0 * Math.PI * time[2]) / (60.0 * 60.0);
    ...
    angle =
      Math.PI / 2.0 -
      (2.0 * Math.PI * time[0]) / 12.0 -
      (2.0 * Math.PI * time[1]) / (60.0 * 12.0);
